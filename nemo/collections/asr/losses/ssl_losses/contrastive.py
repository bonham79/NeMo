...
class ContrastiveLoss(Loss):
    ...
    def __init__(
        ...
        Args:
            ...
            quantizer_cfg: Config dictionary used for quantizer.
    ...
    def add_quantizer(self, quantizer):
        # We're being lazy and just adding this in at different points.
...
class FixedCodebookContrastiveLoss(Loss):
    @property
    def input_types(self):
        """Input types definitions for Contrastive.
        """
    ...
    def forward(self, spectrograms, spec_masks, decoder_outputs, decoder_lengths=None):
        ...
        if self.quantized_targets:
            if self.store_ids:
                # store ids for use by other losses
                targets, self.target_ids = self.quantizer(targets, return_ids=True)
                ...
                targets = self.quantizer(targets)
        else:
            targets = self.target_proj(targets)
